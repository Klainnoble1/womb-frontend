...
export default function Admin() {
  ...
  const [status, setStatus] = useState('Loading admin project desk...');

  useEffect(() => {
    fetch(`${API_URL}/api/projects/admin`)
      .then((res) => res.json())
      .then((data) => {
      ...
      .catch(() => setStatus('Unable to load project desk.'));
  }, []);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-6">
          <div>
            ...
            <p className="text-sm text-slate-400">
              Project submitter contacts and vendor bids are visible here for admin follow-up.
            </p>
          </div>
          <span className="px-3 py-1 rounded-full bg-womb-emerald/20 text-womb-emerald border border-womb-emerald/30 text-xs font-bold w-fit">
